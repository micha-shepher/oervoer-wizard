...
from oervoer.models import Taste, MeatType, Product
from . import logger
...
class ImportOervoer(object):
    ...
    def connect(self):
        if self.conn:
            return True
        try:
            logger.info('connecting to {}@{}'.format(self.user, 'oervoer.com'))
    ...
    def execute(self):
        try:
            ...
            logger.error('cannot obtain cursor')
            return False
...
class ImportProds(ImportOervoer):
    ...
    url = "https://www.oervoer.com/rest//V1/inventory/get-product-salable-quantity/{}/1"

    def updateqty(self, prod: Product, used: int):
        query = '''
        ...
        self.execute()
        self.conn.commit()
        logger.info('setting qty of {0} to {1} in magento'.format(prod.sku,
                                                                  prod.qty-used))

    def get_saleable_qty(self, prod: Product) -> int:
        """

        :param prod: The product whose saleable quantity we need
        :return: saleable quantity from the magento DB
        """
        response = requests.get(self.url.format(prod.sku), headers=
        {'Authorization': 'Bearer h672j2msrx52wxhdghrzclun8phsvhzo',})
        if response.status_code == 200:
            logger.info(f'reading saleable qty {prod}')
            return int(response.text)
        else:
            logger.error(f'not possible to read saleable qty {prod}, '
                         f'code={response.status_code}, text={response.text}')
            return self.readqty(prod)

    def readqty(self, prod: Product) -> int:
        query1 = f'''
        SELECT inv.qty FROM cataloginventory_stock_item as inv
        WHERE {prod} = inv.product_id
        '''

        self.set_query(query1)
        self.execute()
        results = self.cur.fetchall()
        logger.info('reading back qty of {0}: {1} from magento'.format(
            prod, results[0][0]))
            ...
            try:
                ...
                for ind in geschikt:
                    ...
                    if ind in cat_dog:
                        ...
                        logger.error('no key {}'.format(ind))
            except ValueError:
                pass
            prod[-1] = (hond,kat,)
        ...
        for rec in self.results:
            ...
            if kat:
                ...
                if hond:
                    ...
                    logger.error('strange product niet geschikt noch kat noch hond {'
                             '}'.format(rec[2]))
                    continue
...
class ImportOrders(ImportOervoer):
        ...
        for option in options:
            ...
            if option['label'].find('Geboortedatum') > -1:
                ...
                try:
                    ...
                    logger.error('unable to format {}'.format(date))
    ...
    def processResults(self, results):
        ...
        for r in results:
            ...
            s = filter(lambda x: x in string.printable, r[-1])  # last field is options
            s = ''.join([letter for letter in s])
            # field, where the owner, pet name and weight are stored.
            try:
                d = json.loads(self.correct_string_len(s))    # this string (s) is already without strange characters
            except ValueError:
                d = {}
                logger.error('bad options voor {}'.format(str(r[-1])))
            ...
            if 'options' in d:
                ...
                with open('{0}/{1}.deb'.format(tempdir, r[0]),'w') as f:
                    f.write(str(d))
                logger.error(r[0],r[1],r[7],' does not have options!')
                logger.error(d.keys())
