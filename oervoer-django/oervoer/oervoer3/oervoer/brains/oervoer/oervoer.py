...
import numpy
from oervoer.models import Order, Globals, MeatType, Product, PickList
...
class Oervoer(object):
    ...
    def parse_products(self):
        ...
        for prod in self.products:
            prod.ordered = 0
            if prod.vlees in self.prodlists:
    ...
    def dump(self):
        ...
        for i in self.prodlists.keys():
            logger.info("produkt klasse: %s, aantal produkten: %d" %
                        (i, len(self.prodlists[i])))
    ...
    def make_list(self, vlees, order, meal_size):
        ...
        logger.info(f"processing {order.pet}, dontlist={dontslist}")
        ...
        if vlees == 'BOT':
            if order.pet.is_hond():
                for _vlees in (MeatType.objects.filter(
                        meat_type__in=('HARD BOT', 'MIDDEL BOT', 'ZACHT BOT'))):
                    try:
            ...
            if order.pet.is_hond():
                ...
                for _vlees in MeatType.objects.filter(
                        meat_type__in=('COMPLEET KARKAS.ZACHT BOT',
            ...
            for _vlees in MeatType.objects.all():
                if _vlees.is_gemalen:
                    try:
                        thelist += self.prodlists[
                            MeatType.objects.get(meat_type=_vlees)]
                    except KeyError:
                        pass
        elif vlees == 'PENS':
        ...
        #
        thelist.sort(key=lambda
            prod: prod.get_norm_weight())  # work with products sorted by normalized weight
        logger.info('total products in list for {} = {}'.format(vlees, len(thelist)))

        for pr in thelist:
            condition = (meal_size * fact1 >= pr.get_norm_weight() >= meal_size * fact2)
            if pr.vlees.is_gemalen:
                if abs(profile.MEAL - profile.BIGMEAL) < 50 or abs(
                        profile.MEAL - profile.SMALLMEAL) < 50:
                    condition = abs(
                        pr.get_norm_weight() - Decimal(profile.MEAL / 1000.0)) < 0.05
                else:
                    pass

            logger.info('{:>25}{:>3}{:>3}{:>3}{:>3}{:>3}{:>4}'.format(pr.sku,
                                                                      condition,
                                                                      not (
                                                                                  pr.smaak in donts),
                                                                      set(
                                                                          pr.smaak.taste.split(
                                                                              '.')).isdisjoint(
                                                                          set(
                                                                              dontslist)),
                                                                      not (
                                                                                  pr.vlees.meat_type in dontslist),
                                                                      self.product_ok_for_catdog(
                                                                          pr,
                                                                          order.pet.ras.ras),
                                                                      pr.qty))
            if condition:
                if not pr.vlees.meat_type in dontslist:
                    if (not (pr.smaak in donts)) and \
                            (pr.qty > 0) and \
                            set(pr.smaak.taste.split('.')).isdisjoint(
                                set(dontslist)) and \
                            self.product_ok_for_catdog(pr, order.pet.ras.ras):
                        outlist.append(pr)
                    else:
                        logger.info('{} failed qty or dog-cat or taste'.format(pr))
                else:
                    logger.info('{} failed meat type condition, meat type {}'.format(
                        pr, pr.vlees))
            else:
                logger.error('{} failed mealsize {}'.format(pr, pr.get_norm_weight()))

        if len(outlist) == 0:
            # try again, choose products smaller than or greater than...
            logger.error("no products found for {0}".format(vlees))
            if meal_size * fact1 < thelist[0].get_norm_weight():  # too small!
                thelist = [p for p in thelist if
                           p.get_norm_weight() <= (profile.SMALLMEAL + 50) / 1000.0]
            else:
                thelist = [p for p in thelist if
                           p.get_norm_weight() >= (profile.BIGMEAL - 50) / 1000.0]
        ...
        if len(outlist) == 0:
            raise NoProductsException(
                'Lijst van potentieele {} producten voor {} is leeg.'.format(vlees,
                                                                             order.pet))
        for p in outlist:
            if self.random:
                quant = p.get_qty()
        ...
        try:
            ...
            if meattype.is_orgaan:
                ...
                while total_weight < weight and tries < profile.tries:
                    ...
                    if prod.smaak in donts:
                        logger.info('{0} in donts, isliver={1}, rejected!'.format(
                            prod.sku, prod.smaak.is_liver))
                        continue

                    if (
                            total_weight <= weight * profile.LEVERDEEL and prod.smaak.is_liver) or \
                            (
                ...
                if tries >= profile.tries:
                    logger.error(
                        'orgaan error! cant fulfill requirements for {order.pet}')
            ...
            # complete the filling
                ...
                if toomany > profile.REPEATS:
                    logger.error('failed to fill {}, w {}, len {}'.format(vleessoort,
                                                                          total_weight,
                                                                          len(l)))
                    raise NoProductsException(
                        'Kan lijst {0} niet voldoen, misschien pakket te groot of viskop rule?'.format(
                            vleessoort),
                        l, total_weight)
        except ValueError:
    ...
    def choose_best(self, l, qty, to_reach_weight):
        ...
        logger.info('median number of tastes is {}'.format(median))
        ...
        if len(final) == 0:
            logger.error('''something wrong happened when looking for best selection, 
            resulting in no solutions.''')
            logger.info('original length: {}({})'.format(len(l), len(tlist)))
    ...
    def select_type(self, meal_type, order, meal_size, to_reach_weight):
        ...
        l = []
        qty = []
        for _ in range(order.pet.profile.tries):
    ...
    def make_plus(self, order):
        ...
        products_in_order = []
        try:
            total_vis, products_in_order = self.select_type(
                'VIS', order, meal_size, package_rest * vis_factor)
            logger.info('vis factor {0}, total vis {1}'.format(vis_factor, total_vis))
            package_rest -= total_vis
        except NoProductsException as e:
            logger.error('no vis! vis factor {0},  {1}'.format(vis_factor, e))
        ...
        if order.pet.is_hond() and not 'PENS' in [d.taste for d in donts]:
            try:
                total_pens, selection = self.select_type(
                    'PENS', order, meal_size, float(order.weight) * pens_factor)
                logger.info('pens factor {0}, total pens {1}'.format(pens_factor,
                                                                     total_pens))
                products_in_order.extend(selection)
                package_rest -= total_pens
            except NoProductsException as e:
                self.exceptions.append(('PENS', order, e))
                logger.error('no pens! pens factor {0},  {1}'.format(pens_factor, e))
                if e.l:
                    total_pens = e.weight
                    products_in_order = e.l
    ...
    def update_inventory(self, delivery):
        ...
        for picklist in PickList.objects.filter(delivery=delivery):
            ...
            try:
                picklist.product.qty = importobj.get_saleable_qty(picklist.product)
            except:
                logger.error('error reading back qty {0}'.format(picklist.product))
                picklist.product.qty = picklist.product.qty - picklist.number
            picklist.product.save()
            logger.info('{0} was {1}, now {2}, used: {3}'.format(
                picklist.product.sku, was, picklist.product.qty, picklist.number))

    def correct_result(self, order, result):
        ''' try to fix the result by removing products that are not unique
        and make the package too heavy.
        look for products whose weight approximates the difference or
        the total weight from the package weight'''

        weights = [float(i.weight) for i in result]  # weight vector
        pckg = float(order.weight)
        looking4 = abs(
            pckg - sum(weights))  # looking for a product weighing about that.
        if looking4 < 0.05 or sum(weights) < pckg:  # not worth fixing if it ain't broke
            return  # nothing to do
        min_val = min([abs(i - looking4) for i in
                       weights])  # difference between closest product and delta in package.

        # search for candidates for deletion
        for i, j in enumerate(weights):
            if abs(j - looking4) == min_val:
                if [p.sku for p in result].count(
                        result[i].sku) > 1:  # if the product is not unique
                    logger.info(f"removing {result[i]} number {i}")
                    result.pop(i)
                    break
    ...
    def make_100(self, order):
        ...
        total_vis = 0.0
        try:
            total_vis, products_in_order = self.select_type('VIS', order, meal_size,
                                                            package_rest * vis_factor)
            package_rest -= total_vis
        except NoProductsException as e:
            self.exceptions.append(('VIS', order, e))
            if e.l:
        ...
        ##############################
        try:
            total_compleet_karkas, selection = self.select_type(
                'COMPLEET KARKAS', order, meal_size,
        ...
        logger.info('relative part after vis, pens and karkas {0}'.format(
            relative_part))
        try:
            total_orgaan, selection = self.select_type(
                'ORGAAN', order, meal_size, relative_part * orgaan_factor)
            products_in_order.extend(selection)
        ...
        ###################################
        try:
            total_spier, selection = self.select_type(
                'SPIERVLEES', order, meal_size, relative_part * spier_factor)
        ...
        ####################################
        try:
            total_bot, selection = self.select_type(
                'BOT', order, meal_size, package_rest)
        except NoProductsException as e:
        ...
        logger.info(f'vis {total_vis},pens {total_pens},kark {total_compleet_karkas},'
                    f'spier {total_spier}, orgaan {total_orgaan}, bot {total_bot}')
        logger.info(f'vis {total_vis / pkg}, pens {total_pens / pkg}, kark'
                    f' {total_compleet_karkas / pkg},'
                    f' spier {total_spier / pkg}, orgaan {total_orgaan / pkg},'
                    f' bot {total_bot / pkg}')
        return products_in_order
...
if __name__ == '__main__':
    ...
    if not testdir:
        testdir = '../test/'
    oervoer = Oervoer(testdir + 'products.csv', testdir + 'orders.csv',
                      testdir + 'picklists.csv')
