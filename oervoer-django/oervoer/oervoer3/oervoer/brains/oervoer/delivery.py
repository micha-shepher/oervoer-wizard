...
from ...tables import PickListTable
    ...
    def bol(self, csv=False, brieven=False):
        ...
        def seq(vleestype):
            return ['VIS', 'GEMALEN VIS', 'PENS', 'SPIERVLEES', 'ZACHT BOT',
                    'MIDDEL BOT', 'HARD BOT',
                    'COMPLEET KARKAS', 'COMPLEET KARKAS.ZACHT BOT',
                    'COMPLEET KARKAS.MIDDEL BOT', 'COMPLEET KARKAS.HARD BOT',
        ...
        for i in self.delivery:
            if i.sku in deliv:
                ...
                deliv[i.sku] = {'seq': seq(i.vlees.meat_type), 'vleestype': i.vlees,
                                'sku': i.sku, 'name': i.name, 'shelf': i.shelf,
                                'aantal': 1, 'gram': i.weight * 1000}
